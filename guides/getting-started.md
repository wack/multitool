...
This tutorial walks through deploying a simple AWS Lambda function behind an API Gateway. You’ll simulate user traffic to the API, and MultiTool will automatically decide whether to promote or roll back the deployment based on the observed error rate.
...
- [ ] <a href="https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html" target="_blank">AWS CLI installed</a>
  ...
  - [ ] Create and configure an <a href="https://docs.aws.amazon.com/IAM/latest/UserGuide/access-key-self-managed.html#Using_CreateAccessKey" target="_blank">Access Key</a>
...
- A “healthy” version that always returns a `200` status
- A “buggy” version that randomly fails with a `400` status 10% of the time

📝 **Note:** File **must** be named `index.js` to execute correctly.

### Create the healthy version

This version always returns a `200` response.
...
This version introduces a simulated bug by returning a `400` error 10% of the time.
...
Now, load the public URL from Step 6 to use in the next step:
