//! Recursively discover `CHECKS.toml` files beneath the working directory.
//!
...
pub const CHECKS_FILENAME: &str = "CHECKS.toml";

/// Recursively find every file named exactly `CHECKS.toml` under `root`.
///
...
mod tests {
    ...
    fn finds_nested_checks_and_ignores_others() {
        ...
        let root = dir.path();
        fs::write(root.join("CHECKS.toml"), "version = 1\n").unwrap();
        fs::create_dir_all(root.join("sub/deep")).unwrap();
        fs::write(root.join("sub/CHECKS.toml"), "version = 1\n").unwrap();
        fs::write(root.join("sub/deep/CHECKS.toml"), "version = 1\n").unwrap();
        // Non-matching files are ignored, including the retired Markdown format.
        fs::write(root.join("README.md"), "nope").unwrap();
        fs::write(root.join("CHECKS.md"), "# Requirement A\ndo a\n").unwrap();
        fs::write(root.join("sub/checks.toml"), "wrong case").unwrap();
        ...
        assert!(
            found
                .iter()
                .all(|p| p.file_name().unwrap() == "CHECKS.toml")
        );
    }
